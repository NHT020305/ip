...
public class UpdateCommand extends Command {
    ...
    public String execute(Ui ui, TaskManager taskManager, TaskStorage taskStorage) throws
            RepeatedTaskException, InvalidDateTimeFormatException, MeaninglessCommandException {
        Task task = taskManager.getTask(index);
        switch (typeOfUpdate) {
        case "/description" -> task.updateDescription(updatedDetails);
        case "/by" -> task.updateTimeBy(new LocalDateTimeParser(updatedDetails).convertToTime());
        case "/from" -> task.updateTimeFrom(new LocalDateTimeParser(updatedDetails).convertToTime());
        case "/to" -> task.updateTimeTo(new LocalDateTimeParser(updatedDetails).convertToTime());
        default -> throw new MeaninglessCommandException(ErrorMessage.MEANINGLESS_COMMAND);
        }
        taskStorage.saveTasks(taskManager);
        return String.format(ReplyMessage.UPDATE_MESSAGE, taskManager.getTask(index));
    }
